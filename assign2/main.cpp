...
std::string kYourName = "First Last"; // Don't forget to change this!
...
std::set<std::string> get_applicants(std::string filename) {
  ...
  if (file.is_open()) {
    ...
    std::cerr << "Error Openning students.txt" << std::endl;
  }

  return applicants;
}

const std::string getInitialsFromName(const std::string &name) {
...
std::queue<const std::string *> find_matches(std::string name,
  ...
  const std::string matchingInitials = getInitialsFromName(name);
  std::queue<const std::string *> matches;
  for (const auto n : students) {
    auto initials = getInitialsFromName(n);
    if (initials == matchingInitials)
      matches.push(&n);
  }
