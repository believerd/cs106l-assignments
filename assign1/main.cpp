...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  if (file.is_open()) {
    ...
    while (getline(file, line)) {
      ...
      Course course{token[0], token[1], token[2]};
...
void write_courses_offered(std::vector<Course> &all_courses) {
  ...
  // remove offered courses from all courses
  ...
  if (file.is_open()) {
    ...
    std::cerr << "Error Openning Course Offered File." << "\n";
  }
}
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  ...
  if (file.is_open()) {
    ...
    std::cerr << "Failed Openning Course Not Offered File." << "\n";
  }
}
